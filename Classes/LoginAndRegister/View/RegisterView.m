...
- (UIButton *)registerButton{
    if (_registerButton == nil) {
        _registerButton = [Utils returnButtonWithTitle:@"获取验证码"];
...
- (UITextField *)nickTF{
    if (_nickTF == nil) {
        _nickTF = [Utils returnTextFieldWithImageName:@"name" andPlaceholder:@"请输入昵称"];
        [self addSubview:_nickTF];
...
- (UITextField *)passwordTF{
    if (_passwordTF == nil) {
        _passwordTF = [Utils returnTextFieldWithImageName:@"password" andPlaceholder:@"请输入6-12位密码"];
        ...
        [_passwordTF mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.mas_equalTo(-30);
            make.height.mas_equalTo(40);
            make.top.mas_equalTo(self.nickTF.mas_bottom).mas_equalTo(20);
        }];
    }
    return _passwordTF;
}
...
- (void)buttonClickedAction:(UIButton *)button{
    ...
    if ([titleString isEqualToString:@"获取验证码"]) {
        ...
        if ([Utils isMobile:phoneString] == YES) {
            ...
            [Utils toastViewWithTitle:@"请输入正确手机号" andBackColor:[UIColor blackColor] andTextColor:[UIColor whiteColor]];
            return;
        }
    }
    if ([titleString isEqualToString:@"注  册"]) {
        [self endEditing:YES];
        if ([self.captchaTF.text isEqualToString:@""]) {
            [Utils toastViewWithTitle:@"请输入验证码" andBackColor:[UIColor blackColor] andTextColor:[UIColor whiteColor]];
            return;
        }
        //验证密码安全性
        if ([Utils checkPassword:self.passwordTF.text] == NO) {
            [Utils toastViewWithTitle:@"请输入6-12位数字字母组合密码" andBackColor:[UIColor blackColor] andTextColor:[UIColor whiteColor]];
            return;
        }
    }
    _RegisterCallBack(titleString);
}
