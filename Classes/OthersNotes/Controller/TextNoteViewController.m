...
@interface TextNoteViewController ()<UITableViewDelegate, UITableViewDataSource, UITextViewDelegate>

@property (nonatomic) ReadTextNoteView *readView;
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self.readView.commentsTableView.mj_header beginRefreshing];
}
...
- (void)viewDidLoad {
    ...
    self.readView = [[ReadTextNoteView alloc] init];
    [self.view addSubview:self.readView];
    self.readView.noteModel = self.noteModel;
    ...
    self.readView.commentsTableView.delegate = self;
    self.readView.commentsTableView.dataSource = self;
    
    self.readView.commentsTableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        [self findAllZanObjectId];
        [self findAllObjects];
    }];
    
    self.readView.commentsTableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        [self loadMoreNews];
    }];
    
    [self hasAttention];
    
    __block __weak TextNoteViewController *weakself = self;

    [self.readView.readBottomView setReadBottomCallBack:^(id obj) {
        //分享
...
- (void)findAllObjects{//下拉刷新
    ...
    [bq findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
        [self.readView.commentsTableView.mj_header endRefreshing];
        if (error) {
            ...
            self.readView.commentCountLabel.text = [NSString stringWithFormat:@"共%ld条",self.allCommentsArray.count];
            [self.readView.commentsTableView reloadData];
        }
    }];
}
...
- (void)loadMoreNews{//上拉加载
    ...
    [bq findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
        [self.readView.commentsTableView.mj_footer endRefreshing];
        if (error) {
            [Utils toastViewWithError:error];
        }else{
            for (BmobObject *obj in array) {
...
- (void)hasAttention{
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
        if (array.count > 0) {
            //已关注
            [self.readView.readHeadView.focusButton setTitle:@"已关注" forState:UIControlStateNormal];
            [self.readView.readHeadView.focusButton setBackgroundColor:[UIColor lightGrayColor]];
        }
    }];
}
