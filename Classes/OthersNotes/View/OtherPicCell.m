...
- (void)setNoteModel:(NoteModel *)noteModel{
    ...
    if (noteModel.title == nil || [noteModel.title isEqualToString:@""]) {
        ...
        if ([noteModel.type isEqualToString:@"1"] || [noteModel.type isEqualToString:@"3"]) {
            [self.firstImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",noteModel.imagePaths.firstObject]] placeholderImage:[UIImage imageNamed:@"404"]];
        }else if([noteModel.type isEqualToString:@"2"]){
            self.firstImageView.image = [UIImage imageNamed:@"recordPic"];
        }
    }
}
