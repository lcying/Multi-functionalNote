 ...
 *   注册通知  
 */
+ (void)registerLocalNotification:(NSInteger)time content:(NSString *)content key:(NSString *)key repeat:(NSInteger)repeat{
    //创建一个本地通知
    ...
    // 设置重复的间隔
    notification.repeatInterval = repeat;//0表示不重复
    if (repeat != 0) {
