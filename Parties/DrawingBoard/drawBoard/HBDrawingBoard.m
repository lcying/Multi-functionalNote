...
- (void)eraser
{
    if (!self.ise) {
        ...
        self.lineWidth = _lastLineWidth * 10;
...
//    UIGraphicsBeginImageContextWithOptions(CGSizeMake(screen_size.width * [UIScreen mainScreen].scale, screen_size.height * [UIScreen mainScreen].scale), NO, 0.0);
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(screen_size.width, screen_size.height - 104), NO, 0.0);
...
//    UIImageWriteToSavedPhotosAlbum(getImage, nil, nil, nil);
    ...
    //保存到自建相册
    [Utils phAuthorizationCheckWithImage:getImage];
    
    [Utils toastview:@"已保存至随便笔记相册"];
}
